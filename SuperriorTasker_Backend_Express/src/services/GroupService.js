...
class GroupService {
  ...
  async getAllUserGroups(userId, pageable) {
    const memberships = await userGroupRelationService.getMembershipsByUserId(
      userId,
      pageable
    );
    ...
    const groupPromises = memberships.map(async (membership) => {
      ...
      if (!group) {
        throw new Error(
          "There is a relation between user and a group that doesn't exist"
        );
      }
  ...
  async getGroupMembers(groupId, pageable, req) {
    ...
    const memberships = await userGroupRelationService.getMembershipsByGroupId(
      groupId,
      pageable
    );
  ...
  async promoteUser(changeGroupAdminDto) {
    ...
    if (membership.role !== changeGroupAdminDto.role) {
      ...
      console.log(
        `Role ${changeGroupAdminDto.role} assigned to user ${changeGroupAdminDto.userId} in group ${changeGroupAdminDto.groupId}`
      );
    } else {
      console.log(
        `User ${changeGroupAdminDto.userId} already has role ${changeGroupAdminDto.role} in group ${changeGroupAdminDto.groupId}`
      );
    }
  }
}
