...
router.post('/createGroup', upload.single('photoUri'), async (req, res) => {
    try {
        ...
        const response = await GroupService.createGroup(name, description, photoFile, req);
...
router.patch('/change-role-for-group-user', async (req, res) => {
    try {
        ...
        await GroupService.promoteUser(req.body);
        res.status(200).json({ message: 'Successfully changed role of the user for this group...' });
    } catch (error) {
...
router.patch('/editGroup', upload.single('file'), async (req, res) => {
    try {
        ...
        if (!isAdmin) {
            return res.status(401).json({ message: 'Unauthorized: User is not an admin of this group' });
        }

        const response = await GroupService.editGroupInfo(groupId, name, description, file);
        res.status(200).json(response);
    } catch (error) {
...
router.post('/add-new-user-to-group', async (req, res) => {
    try {
        ...
        const response = await UserGroupRelationService.createNewUserGroupRelation(userId, groupId);
        console.log("User added to the group...")
        res.status(200).json(response);
    } catch (error) {
        console.error('Error adding user to group:', error);
        if (error.message.includes('No group associated') || error.message.includes('User doesn\'t exist')) {
            res.status(404).json({ message: error.message });
...
router.post('/add-users-to-group', async (req, res) => {
    try {
        ...
        const response = await UserGroupRelationService.createMultipleUserGroupRelations(users, groupId);
        res.status(200).json(response);
    } catch (error) {
...
router.delete('/leave-group', async (req, res) => {
    try {
        ...
        const response = await UserGroupRelationService.removeUserFromGroup(userId, groupId, false, req);
...
router.delete('/kick-from-group', async (req, res) => {
    try {
        ...
        const response = await UserGroupRelationService.removeUserFromGroup(userId, groupId, true, req);
        res.status(200).json({ message: response });
    } catch (error) {
